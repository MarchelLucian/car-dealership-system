document.addEventListener("keydown", function (e) {
    if (e.key === "Enter") {
        ...
        // Blochează submit-ul formularului
        // DAR nu bloca execuția handlerelor tale
...
function validateAddSale(event) {
    ...
    if (!masinaId.value) {
        ...
        // evidențiem ambele câmpuri de căutare
    ...
    if (!clientId.value) {
        document.getElementById("error-client-validation")
            .textContent = "Please search and select a valid client.";
        document.getElementById("buyerCuiCnpInput")
    ...
    if (!price.value) {
        document.getElementById("error-price").textContent = "Final price is required.";
    ...
    if (!payment.value) {
        document.getElementById("error-payment").textContent = "Select payment method.";
    ...
    if (!date.value) {
        document.getElementById("error-date").textContent = "Sale date is required.";
...
function searchCarByVin() {
    ...
    fetch(`/agent-dashboard/sales/lookup-car-by-vin?vin=${encodeURIComponent(vin)}`)
        .then(res => res.json())
        .then(data => {
...
function searchClientByCuiCnp() {
    ...
    setTimeout(() => {
    fetch(`/agent-dashboard/sales/lookup-client?cuiCnp=${encodeURIComponent(cuiCnp)}`)
        .then(res => res.json())
        .then(data => {
